import { RailwayError, AIError } from "@/types";
...
export class DeepSeekAI {
  ...
  constructor() {
    ...
    if (!this.apiKey) {
      console.warn(
        "Warning: DEEPSEEK_API_KEY not found. AI suggestions will be disabled."
      );
    }
  }

  async generateSuggestion(error: RailwayError): Promise<string> {
    if (!this.apiKey) {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a Railway deployment expert. Provide concise, actionable fix suggestions for deployment errors. Focus on practical solutions that users can implement immediately.",
            },
            {
              role: "user",
              content: prompt,
  ...
  async analyzeLogsWithAI(logContent: string): Promise<AIError[]> {
    if (!this.apiKey) {
      return [];
    }
...
If no errors are found, return an empty array [].`;
    ...
    try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a Railway deployment expert. Analyze logs and identify errors. Always respond with valid JSON format as requested.",
            },
            {
              role: "user",
              content: prompt,
      ...
      if (response.ok) {
        ...
        try {
          ...
          const aiErrors = JSON.parse(cleanedContent);
  ...
  private buildPrompt(error: RailwayError): string {
    return `
...
`;
  ...
  private getFallbackSuggestion(error: RailwayError): string {
...
`,
    ...
    return (
      fallbackSuggestions[error.error_type] ||
      `
